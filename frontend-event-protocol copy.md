...
**vnc 字段说明：**
- 当工具调用涉及 VNC 显示时携带此字段
- `app`: 应用名称（terminal/chrome等）
- `display`: X display 号
- `novnc_port`: noVNC WebSocket 端口
- `vnc_wait_id`: 等待ID，前端连接VNC成功后需发送此ID通知后端

**前端处理 VNC 连接：**
```javascript
ws.on('tool_call', (data) => {
  if (data.payload.vnc && data.payload.vnc_wait_id) {
    const vncUrl = `wss://vnc.example.com/websockify?session=${sessionId}:${data.payload.vnc.app}`;
    ...
    rfb.addEventListener('connect', () => {
      ws.send({
        type: 'vnc_connected',
        payload: { vnc_wait_id: data.payload.vnc_wait_id }
      });
    });
  }
});
...
**vnc 字段说明：** 同 `tool_call`，标识此工具结果关联的 VNC 会话
